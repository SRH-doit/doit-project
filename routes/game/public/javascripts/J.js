...
function checkWord() {
  if (
    standardWord.charAt(standardWord.length - 1) === comparisonWord.charAt(0)
  ) {
...
function enterWord(event) {
  ...
  !standardWord
    ? (standardWord = word_input.value)
    : ((comparisonWord = word_input.value), checkWord());
  noticeTurn();
  displayWord();
  word_input.value = "";
}
...
function saveNickname() {
  const nickname_input = document.querySelectorAll(
...
function SwitchToggle() {
  toggleResetBtn
    ? (checkNumber(), propertySet.trueToggle())
    : propertySet.falseToggle();
}
